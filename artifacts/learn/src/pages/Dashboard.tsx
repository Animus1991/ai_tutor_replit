    ...
    <Card className="bg-card border-border">
      <CardContent className="p-5">
        <div className="flex items-start justify-between">
          ...
          <div
            className={`h-9 w-9 rounded-xl flex items-center justify-center ${color || "bg-primary/10"}`}
          >
            <Icon
              className={`h-5 w-5 ${color ? "text-white" : "text-primary"}`}
            />
          </div>
        </div>
      </CardContent>
...
export default function DashboardPage() {
  ...
  const recentActivity = activity as
    | Array<{
        id: number;
        activityType: string;
        description: string;
  ...
  const mMeta =
    MASTERY_META[model?.masteryLevel ?? "developing"] ??
    MASTERY_META.developing;
    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            How exam-ready you are, at a glance
          </p>
        </div>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-xs text-muted-foreground">
                      {Math.round((model?.confidence ?? 0) * 100)}% confidence
                    </span>
                  </div>
                  <p className="text-sm text-muted-foreground max-w-sm">
                    Built from your real performance — accuracy, hint reliance
                    and practice volume.
                  </p>
                  {model?.calibration && (
                    <p className="text-xs mt-2">
                      ...
                      <span
                        className={
                          CAL_META[model?.calibration.direction]?.cls ??
                          "text-muted-foreground"
                        }
                      >
                        {CAL_META[model?.calibration.direction]?.label ??
                          model?.calibration.direction}{" "}
                        · {model?.calibration.score}/100
                      </span>
                    </p>
                  )}
                </div>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-5 text-center sm:text-left">
              ...
              <div>
                <p className="font-semibold text-foreground">
                  Your Exam Readiness score is warming up
                </p>
                <p className="text-sm text-muted-foreground mt-1">
                  Answer {model?.nextInsightAt ?? 5} more graded questions and
                  we'll score how exam-ready you are — from real performance,
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          sub={`${stats?.inProgressCourses ?? 0} in progress`}
        ...
        <StatCard
          ...
          sub={`${stats?.weeklyXp ?? 0} XP this week`}
          color="bg-orange-500"
        />
        <StatCard
          icon={Zap}
            ...
            <Card className="bg-card border-border border-dashed">
              <CardContent className="p-12 text-center">
                ...
                <p className="text-muted-foreground mb-4">
                  No courses yet. Upload some notes to get started.
                </p>
                <Button asChild>
                  <Link href="/notes">
                    <FileText className="h-4 w-4 mr-2" />
