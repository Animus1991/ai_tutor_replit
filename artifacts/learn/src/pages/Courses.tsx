...
export default function CoursesPage() {
  ...
  const allCourses = courses as Array<{
    id: number; title: string; description: string; status: string; difficulty: string;
    courseType: string; totalSteps: number; estimatedMinutes: number; quizFrequency: string; createdAt: string;
  }> | undefined;
    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Your AI-generated interactive lessons</p>
        </div>
        <Button asChild>
      ...
      {isLoading ? (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {[...Array(6)].map((_, i) => <div key={i} className="h-52 bg-white/5 rounded-xl animate-pulse" />)}
        </div>
      ) : !allCourses?.length ? (
        <Card className="border-dashed border-border bg-transparent">
          <CardContent className="p-20 text-center">
            ...
            <div className="flex gap-3 justify-center">
              ...
              <Button asChild><Link href="/courses/new">Create Course</Link></Button>
            </div>
          </CardContent>
        </Card>
      ) : (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered?.map((course) => (
            <Card key={course.id} className="bg-card border-border hover:border-primary/30 transition-colors group relative">
              <CardContent className="p-5 flex flex-col h-full">
                <div className="flex items-start gap-2 mb-2">
                  <Link href={`/courses/${course.id}`} className="flex-1 min-w-0">
                    <h3 className="font-semibold text-foreground group-hover:text-primary transition-colors line-clamp-2 leading-snug">{course.title}</h3>
                  </Link>
                  <button
                ...
                <div className="flex flex-wrap gap-1.5 mb-4">
                  <Badge variant={statusVariant(course.status)} className="text-xs capitalize">
                    {course.status === "generating" && (
                      <span className="inline-block h-1.5 w-1.5 rounded-full bg-current mr-1.5 animate-pulse" />
                    )}
                    {course.status}
                  </Badge>
                  <Badge variant="outline" className={`text-xs capitalize ${difficultyColor(course.difficulty)}`}>{course.difficulty}</Badge>
                  <Badge variant="outline" className="text-xs capitalize">{course.courseType}</Badge>
                </div>

                <div className="mt-auto flex items-center justify-between text-xs text-muted-foreground">
                  <span className="flex items-center gap-1"><Clock className="h-3 w-3" />{course.estimatedMinutes}min</span>
                  {course.totalSteps > 0 && (
                    <span className="flex items-center gap-1"><Zap className="h-3 w-3" />{course.totalSteps} steps</span>
                  )}
                  <Link href={`/courses/${course.id}`} className="text-primary hover:underline font-medium">
      ...
      <Dialog open={deleteId !== null} onOpenChange={() => setDeleteId(null)}>
        <DialogContent className="sm:max-w-sm">
          ...
          <p className="text-sm text-muted-foreground">This will permanently delete the course and all lesson progress. This cannot be undone.</p>
