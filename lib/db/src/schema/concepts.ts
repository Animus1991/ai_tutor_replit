...
export const conceptEdgesTable = pgTable("concept_edges", {
  id: serial("id").primaryKey(),
  courseId: integer("course_id").notNull().references(() => coursesTable.id, { onDelete: "cascade" }),
  prerequisiteConceptId: integer("prerequisite_concept_id").notNull().references(() => conceptsTable.id, { onDelete: "cascade" }),
  dependentConceptId: integer("dependent_concept_id").notNull().references(() => conceptsTable.id, { onDelete: "cascade" }),
  strength: real("strength").notNull().default(1),
}, (t) => ({
  uniq: uniqueIndex("concept_edges_uniq").on(t.prerequisiteConceptId, t.dependentConceptId),
...
export const lessonStepConceptsTable = pgTable("lesson_step_concepts", {
  id: serial("id").primaryKey(),
...
export const insertConceptEdgeSchema = createInsertSchema(conceptEdgesTable).omit({ id: true });
...
export const insertLessonStepConceptSchema = createInsertSchema(lessonStepConceptsTable).omit({ id: true });
export type InsertLessonStepConcept = z.infer<typeof insertLessonStepConceptSchema>;
