...
export const masteryRecordsTable = pgTable(
  ...
  (t) => ({
    uniq: uniqueIndex("mastery_records_user_concept_uniq").on(
      t.userId,
      t.conceptId,
    ),
...
export const mistakesTable = pgTable(
  "mistakes",
  {
    ...
    answerEventId: integer("answer_event_id").references(
      () => answerEventsTable.id,
      { onDelete: "set null" },
    ),
    conceptId: integer("concept_id").references(() => conceptsTable.id, {
      onDelete: "set null",
    }),
...
export const reviewSchedulesTable = pgTable(
  ...
  (t) => ({
    uniq: uniqueIndex("review_schedules_user_concept_uniq").on(
      t.userId,
      t.conceptId,
    ),
...
export const insertMasteryRecordSchema = createInsertSchema(
  masteryRecordsTable,
...
export const insertReviewScheduleSchema = createInsertSchema(
